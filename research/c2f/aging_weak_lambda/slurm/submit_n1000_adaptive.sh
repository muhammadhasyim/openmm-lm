#!/usr/bin/env bash
# Archive existing outputs, rebuild CUDA plugin, pilot then full N=1000 adaptive submit.
set -euo pipefail
...
SBATCH="${SLURM_DIR}/14_production_adaptive_n1000.sbatch"
...
for pattern in cavity-aging cavity-lam003-resubmit cavity-n1000-adapt; do
...
if [[ "${MODE}" == "pilot" ]]; then
  echo "=== Submit pilot: array=0, STACK=1 (1 replica, 5 lambda, jobs=5) ==="
  JOB_ID=$(STACK=1 sbatch --parsable --array=0 "${SBATCH}")
  echo "Pilot job ${JOB_ID}"
  echo "Monitor: tail -f ${LOG_DIR}/n1000_adapt_${JOB_ID}_0.out"
  echo ""
  echo "After pilot completes, check wall time and stability:"
  echo "  ${PY} validate_replica_stability.py --replica-start 0 --replica-end 0"
  echo "Then submit full campaign:"
  echo "  bash ${SLURM_DIR}/submit_n1000_adaptive.sh full"
  exit 0
fi
...
echo "=== Submit full N=1000 campaign: array=0-249%4, STACK=4 ==="
JOB_ID=$(sbatch --parsable --array=0-249%4 "${SBATCH}")
