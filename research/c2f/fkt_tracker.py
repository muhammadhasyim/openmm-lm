...
class FKTTracker:
    ...
    def to_state_dict(self) -> dict:
        """Serialize tracker state for checkpoint resume."""
        references = []
        for ref in self.references:
            references.append(
                {
                    "time_ps": float(ref["time_ps"]),
                    "ref_file_idx": int(ref["ref_file_idx"]),
                    "rhok_real": np.asarray(ref["rhok_real"], dtype=np.float64),
                    "rhok_imag": np.asarray(ref["rhok_imag"], dtype=np.float64),
                }
            )
        written_log_points = {
            int(file_idx): sorted(int(i) for i in indices)
            for file_idx, indices in self._written_log_points.items()
        }
        return {
            "kmag_nm_inv": float(self.kmag_nm_inv),
            "num_wavevectors": int(self.num_wavevectors),
            "reference_interval_ps": float(self.reference_interval_ps),
            "max_references": int(self.max_references),
            "output_period_ps": float(self.output_period_ps),
            "log_time_spacing": bool(self.log_time_spacing),
            "min_log_ps": self.min_log_ps,
            "max_log_ps": self.max_log_ps,
            "log_num_points": int(self.log_num_points),
            "wavevectors": np.asarray(self.wavevectors, dtype=np.float64),
            "references": references,
            "last_reference_time_ps": self.last_reference_time_ps,
            "last_output_time_ps": self.last_output_time_ps,
            "next_ref_file_idx": int(self._next_ref_file_idx),
            "written_log_points": written_log_points,
        }

    @classmethod
    def from_state_dict(cls, state: dict, output_prefix: str) -> FKTTracker:
        """Restore tracker from checkpoint state; existing FKT files are appended."""
        tracker = cls(
            kmag_nm_inv=float(state["kmag_nm_inv"]),
            num_wavevectors=int(state["num_wavevectors"]),
            reference_interval_ps=float(state["reference_interval_ps"]),
            max_references=int(state["max_references"]),
            output_period_ps=float(state["output_period_ps"]),
            output_prefix=output_prefix,
            log_time_spacing=bool(state.get("log_time_spacing", False)),
            min_log_ps=state.get("min_log_ps"),
            max_log_ps=state.get("max_log_ps"),
            log_num_points=int(state.get("log_num_points", 50)),
        )
        tracker.wavevectors = np.asarray(state["wavevectors"], dtype=np.float64)
        tracker.references = []
        ...
        tracker.last_reference_time_ps = state.get("last_reference_time_ps")
        tracker.last_output_time_ps = state.get("last_output_time_ps")
        tracker._next_ref_file_idx = int(state["next_ref_file_idx"])
        tracker._written_log_points = {
            int(file_idx): set(int(i) for i in indices)
            for file_idx, indices in state.get("written_log_points", {}).items()
        }
        if tracker.log_time_spacing:
            tracker.log_time_points_ps = np.logspace(
                np.log10(tracker.min_log_ps),
                np.log10(tracker.max_log_ps),
                tracker.log_num_points,
                dtype=np.float64,
            )
        return tracker
