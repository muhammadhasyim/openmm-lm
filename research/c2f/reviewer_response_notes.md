...
(`research/c2f/`). Reference protocol repo: `cav-hoomd/`.
...
```bash
pixi run --as-is -e test python research/c2f/analyze_reviewer_response.py --burn-in 0.2
pixi run --as-is -e test python research/c2f/analyze_material_time.py
```

New simulation campaign (Calc 2 frequency sweep + Calc 4 50 K / new couplings),
launched detached:
```bash
setsid bash -c 'EQ_SKIP_CUDA_REBUILD=1 exec bash research/c2f/run_reviewer_response_sims.sh' &
```
