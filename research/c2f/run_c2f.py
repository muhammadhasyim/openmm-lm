...
from openmm.cavitymd.controllers import PIDControl
from openmm.cavitymd.empirical import EmpiricalTemperatureData
from openmm.cavitymd.feedback import GradientDescentFeedback
from openmm.cavitymd.simulation import (
...
from openmm.cavitymd.trackers import (
    ElapsedTimeTracker,
    EnergyTracker,
    TemperatureTracker,
...
_OPENMM_CALIBRATION_FILE = _SCRIPT_DIR / "calibration_output" / "calibration_data.txt"
REFERENCE_CALIBRATION_FILE = (
    _OPENMM_CALIBRATION_FILE
    if _OPENMM_CALIBRATION_FILE.exists()
    else _SCRIPT_DIR / "reference_potential_energy_vs_T.txt"
)

# ---------------------------------------------------------------------------
#  Physical constants & unit conversions (from openmm.cavitymd.constants.Units)
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--smoke", action="store_true",
                        help="Short CI smoke run (skip calibration, 5 ps production)")
    args = parser.parse_args()

    if args.smoke:
        args.skip_calibration = True
        args.runtime_ps = min(args.runtime_ps, 5.0)
        args.equil_ps = min(args.equil_ps, 1.0)
        args.output_prefix = "smoke_c2f"
