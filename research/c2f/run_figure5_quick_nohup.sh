...
#   nohup bash research/c2f/run_figure5_quick_nohup.sh &
set -euo pipefail
...
LOG_DIR="$ROOT/research/c2f/fig5_output"
mkdir -p "$LOG_DIR"
...
OPENMM_PLUGIN_DIR="$OPENMM_PLUGIN_DIR" pixi run -e test python -u research/c2f/reproduce_figure5.py --quick >> "$LOG_FILE" 2>&1
